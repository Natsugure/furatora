...
import { stations, lines, stationLines, stationConnections, odptMetadata, operators } from '@stroller-transit-app/database/schema';
...
// odptStationId からスラッグを生成
// 例: "odpt.Station:TokyoMetro.Ginza.Shibuya" → "tokyometro-ginza-shibuya"
...
async function updateOdptData(operator: Operator) {
  ...
  await db.transaction(async (tx) => {
    ...
    const lineValues = railwayData.map((railway) => ({
      ...
      slug: generateLineSlug(railway['owl:sameAs']),
    ...
    const stationValues = stationData.map((station) => ({
      ...
      slug: generateStationSlug(station['owl:sameAs']),
    ...
    const connectionValues: {
      ...
      connectedRailwayId: string | null;
    ...
    for (const stationRecord of stationRecords) {
      await tx
        ...
        .where(eq(stationConnections.stationId, stationRecord.id));
    }

    // 新しい接続情報を登録
    if (connectionValues.length > 0) {
      for (let i = 0; i < connectionValues.length; i += BATCH_SIZE) {
        const batch = connectionValues.slice(i, i + BATCH_SIZE);
        await tx.insert(stationConnections).values(batch);
      }
      console.log(`${operator}: Registered ${connectionValues.length} station connections`);
    }
