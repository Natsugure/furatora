---
description: '仕様駆動ワークフローv2は、要件が明確に定義され、設計が綿密に計画され、実装が徹底的に文書化および検証されることを保証する、構造化されたソフトウェア開発アプローチを提供します。v1からの変更点は、ドキュメントを「使い捨て（spec）／恒久（domain）／不変（ADR）」の3種に分離し、それぞれの置き場と更新方法を規定したことです。'
applyTo: '**'
---
# 仕様駆動ワークフロー v2
...
**すべてのドキュメントは、以下のいずれかに分類してから書く。**
分類を誤ると、恒久的に必要な知識が使い捨ての文書に紛れて消える。

| 種類 | 問い | 置き場 | 更新方法 | 寿命 |
|---|---|---|---|---|
| **仕様（spec）** | この作業で**何をするか** | `docs/spec/` | 全面書き換え | **Issue単位。次のIssueで消える** |
| **ドメイン（domain）** | システムは**いまどうなっているか** | `docs/domain/` | **上書き** | 恒久。常に現在を指す |
| **決定（ADR）** | **なぜ**その設計を選んだか | `docs/adr/` | 追記のみ | 不変。覆すときは新ADR + `Superseded by` |
...
この3点セットは**Issueごとに全面的に書き換えられる**。
過去分は git 履歴にのみ残る。したがって、**次のIssue以降も有効であり続ける内容を
ここに置いてはならない。** 該当するものは `docs/domain/` か `docs/adr/` へ移す。

### ドメイン（`docs/domain/`）

システムの現在の姿を記述する。運用ルールは
[`docs/domain/README.md`](../../docs/domain/README.md) に従う。

> **追記しない。上書きする。**

- 「かつてこうだった」は書かない（git 履歴か ADR を見る）
- 検討過程・却下案・その根拠は書かない（ADR の役割）
- 実装が追いついていない期間は、冒頭に**適用状況**を明記する

### 決定（`docs/adr/`）

形式と採番規約は [`docs/adr/README.md`](../../docs/adr/README.md) に従う。

**ADRにするかの判定基準:**

```
その決定は、覆すときに明示的な意思決定を必要とするか？
├─ YES → ADR
└─ NO  → ADR ではない
    ├─ 外部要因で自動的に覆るもの（対象範囲の拡大、依存先の仕様変更など）
    │   → 「予定された作業」である。GitHub Issue に起票し、
    │      現在の制約として docs/domain/ に書く
    └─ この作業限りの判断 → docs/spec/design.md
```

**却下した選択肢とその根拠が無い決定は、ADRにしない。** ADR の価値は
「何を選んだか」ではなく「なぜ他ではないか」にある。

**言語ルール:**
すべてのドキュメンテーション成果物（`docs/spec/`、`docs/domain/`、`docs/adr/` 配下）
およびテンプレートは日本語で生成する必要があります。
...
この形式で記録した決定のうち、上記の判定基準を満たすものは
`docs/adr/` へ MADR 形式で昇格させる。
...
- [ ] **`docs/domain/` と `docs/adr/` を先に読む。**
      - `docs/domain/` は現在のシステムの姿であり、**再導出しない**。
        記述と実コードが食い違う場合は、どちらが正しいかを確定してから進む。
      - `docs/adr/` の既存決定に反する設計を検討する場合は、
        supersede が必要である旨をこの時点で記録する。
...
- [ ] **`design.md`に技術設計を文書化する:**
  ...
  - **ADRの結論は反映するが、根拠は重複させずADRを参照する。**

- [ ] **恒久知識と決定を分離する:**
  - 設計中に確定した内容のうち、**次のIssue以降も有効であり続けるもの**
    （座標系・単位・不変条件・ドメインルール等）を列挙する。
  - 上記の判定基準で、ADR にするもの／`docs/domain/` に置くものを振り分ける。
  - **`docs/domain/` への反映はフェーズ5で行う。** この時点では対象を確定するだけ。
    実装前に「現在の姿」を書き換えないこと。
...
- [ ] **`tasks.md`に実装計画を作成する:**
  ...
  - **フェーズ5のドキュメント更新タスクを計画に含める**
    （`docs/domain/` の更新、適用状況注記の除去、ADRのステータス更新）。
...
- [ ] 意味のあるコメントを追加する。
      ...
      - **恒久的な制約は、それを変更しようとする人が必ず見る場所に置く。**
        スキーマの一意制約・不変条件には、理由と参照先を添える。
- [ ] 計画に従ってファイルを作成する。
      - ファイル作成ログを文書化する。
...
- [ ] **`docs/domain/` を実装後の姿に更新する。**
      - フェーズ2で振り分けた対象を反映する。**追記ではなく上書き。**
      - 実装完了により正しくなった記述の**適用状況注記を外す**。
      - 実装を通じて設計から変わった点（既定値の調整、制約の追加等）を反映する。
      - **該当ファイルが1つも変わらなかった場合、その理由を明記する**
        （「ドメインルールに変更なし」で構わないが、確認したことを残す）。
- [ ] **`docs/adr/` のステータスを更新する。**
      - 実装・検証を通過した ADR を `Proposed` → `Accepted` にする。
      - 前提条件付きのADRは、条件が満たされたかを確認し、
        未達なら差し戻し判断を記録する。
...
- **`docs/domain/` の更新確認を省略しない。** これを飛ばすと、
  恒久知識が `docs/spec/` に取り残されたまま次のIssueで消える。
...
- [ ] プルリクエストを準備する（該当する場合）:
    ...
    5. **`docs/domain/` と `docs/adr/` の変更点**（新規・更新・ステータス変更）。
- [ ] **恒久知識の取り残しを確認する。**
      - `docs/spec/` の3点セットは次のIssueで全面的に書き換えられる。
        次も有効な内容が `docs/domain/` または `docs/adr/` に移っていることを確認する。
      - 予定された将来作業は GitHub Issue に起票し、
        現在の制約を `docs/domain/` に記載したことを確認する。
- [ ] ワークスペースを最終化する。
      - 一時ファイル・スクラッチ・作業ログを削除する。
- [ ] 次のタスクに進む。
      - 移行または完了を文書化する。
...
- **ドキュメンテーション**: 不完全、古い、または欠落しているドキュメントを追跡する。
  - **`docs/domain/` の記述と実コードの乖離は、技術的負債として扱う。**
