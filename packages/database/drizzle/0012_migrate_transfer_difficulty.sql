-- 旧 station_connections の評価済み17行を、乗換難易度の新モデルへ書き直す（Issue #123 / docs/spec/design.md「移行データ表」）
--
...
-- 0件を「そのまま通れる」と読んではならない（design.md 決定5）。#124・#125 の制約になる。
--
-- 【備考の扱い】淡路町↔新御茶ノ水の「隣の大手町駅のほうが便利です」は、出発地に依存する経路上の選好であり
-- 移さない（#30 決定1・決定9）。本郷三丁目の「隣の後楽園・春日駅の乗り換えであれば…一長一短です」は、
-- 開発者判断で接続の notes に残した（design.md 決定2）。ほかの備考は、フラグ・設備の種類で表せるものは
...
BEGIN
  ...
  -- 全方面共通は NULL で表さず、4接続が同一ルートを参照することで表す（#30 決定4）。
