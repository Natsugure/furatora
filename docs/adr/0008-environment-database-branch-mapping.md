...
`apps/web` の `build` から seed を外す。preview ブランチは `main` の
コピーオンライトであり**最初からデータが入っている**ため、
...
本ADRは `apps/web` を配信する Vercel プロジェクトを対象とする。
**`apps/admin` を Vercel にデプロイする場合、同じ設定を適用すること。**
ただし2つの Vercel プロジェクトが同一の preview ブランチに対して
同時に `db:migrate` を走らせると競合する。その時点で、
どちらか一方のみがマイグレーションを担う構成へ変更する必要がある。

### 見直し条件

- DB に個人情報を保存するようになったとき
  （preview ブランチへの本番データ複製を再評価する）
- `apps/admin` を Vercel にデプロイし、マイグレーションの実行主体が
  2つになったとき
