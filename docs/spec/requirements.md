...
- 旧8ページを削除し、駅一覧の「管理」リンクを新ページへ向ける（PR5。admin は認証必須の
  内部ツールで外部リンク・ブックマークの流入が無いため、中継用のリダイレクトは置かない）
...
- 受け入れ基準:
  ...
  - 旧ルート（`/facilities`、`/facilities/*/edit`、`/platforms/*/edit`、
    `/platforms/*/stop-patterns/*`）は削除され、駅一覧の「管理」リンクが
    `/layout` を指す（リダイレクトは置かない。admin は認証必須の内部ツールで
    外部リンク・ブックマークの流入が無く、中継ファイルを残す利得より
    恒久的な死にコードの方が高くつくため）
