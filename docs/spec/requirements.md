...
| `station_lines` | 481 | **複数路線を持つ駅は0件（完全に1:1）** |
...
| ファイル | 行数（現役のみ） | 一意キー |
|---|---|---|
| `company` | 176 | `company_cd` |
| `line` | 602 | `line_cd` |
| `station` | 10,465 | `station_cd`（路線×駅粒度） |
| `join` | 10,189 | `(line_cd, station_cd1, station_cd2)` |

- `station_g_cd` は**乗換駅グループ**を表す。現役駅で 8,766グループ、
  複数駅を含むグループ 1,178件、有向ペア 5,876件
- `station_cd` の上位桁は `line_cd` と一致しない（137件の例外）。`line_cd` 列を使うこと
- `station_g_cd` に13件のダングリング参照がある（存在しない `station_cd` を指す）
...
### US-5: 開発者として、直通運転を後から表現できる状態にしたい

- **REQ-5.1**: システムは運行系統を、順序付きの区間の並びとして表現できること
- **REQ-5.2**: 区間を定義する際、システムは路線の全区間と
  一部区間（開始駅・終了駅を指定）の双方を表現できること
...
### C-1: 新幹線の駅は会員版CSVの内容に依存する

無料版には12路線すべてで駅データが存在しない。会員版で供給される前提で設計するが、
供給されない場合、現行DBの新幹線11駅は `ekidataStationCd` を NULL のまま残す。
この11駅は `odptStationId` を保持するため、後から解決できる。
