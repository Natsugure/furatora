...
- [x] **2026-09-14追記（開発者確認）**: `development`に表参道 東京メトロ半蔵門線3番線
      （10両編成、1号車の先頭200m〜10号車の末尾0m、反転編成）が存在することが判明。
      Chrome DevTools MCP経由でブラウザ手動確認: 5号車と6号車の境界スライダーを
      キーボード操作（フォーカス+矢印キー）で100m→99.5mへ動かしたところ、
      隣接する5号車・6号車の境界のみが更新され、他の境界（1/2=180m・2/3=160m・
      3/4=140m・4/5=120m・6/7=80m・7/8=60m・8/9=40m・9/10=20m）は無変化のまま
      保たれることを確認した。境界ハンドルの図上表示位置も「6」「5」の号車ボックスの
      間という正しい位置に描画されており、`isDoorOrderReversed()`による向き判定
      （PR3のバグ修正、docs/domain/train-stop-patterns.md参照）が実データでも
      正しく機能していることを確認した。確認後は値を100mに戻し保存せず破棄したため
      `development`のデータへの書き込みは無い
- [x] **2026-09-14追記**: `e2e/station-layout.spec.ts` に反転編成ドラッグ系E2Eを追加した。
      表参道 東京メトロ半蔵門線3番線を対象に、5号車と6号車の境界ハンドルをマウスで
      実際にドラッグし、対象の境界は変化する一方で無関係な境界（1/2号車・9/10号車）は
      無変化のまま保たれることを検証する。保存は行わず（ページ遷移で破棄）、
      `development`への書き込みは無い。表参道は東京メトロ銀座線・半蔵門線・千代田線の
      3路線が同名駅として登録されており、既存の`findStationId`（駅名のみで検索し
      最初の行を採用）では路線を一意に絞れなかったため、路線名でも絞り込む
      `findStationIdByLine`を新設した。`pnpm --filter @furatora/admin exec playwright
      test e2e/station-layout.spec.ts`（既存7件＋新規1件、計8件）と`typecheck`で
      確認しすべてpass
...
- [x] 複製 → x をずらす → 保存が1画面で完結すること（2026-09-14、Chrome DevTools MCP
      経由でNeon `development`に対しブラウザ手動確認。渋谷 東京メトロ銀座線1番線に
      テスト用コンコース「中央改札」を作成→複製すると、アクセス点のx座標が
      100m→102m（+2m）にずれた状態で編集フォームに反映され、そのまま保存すると
      `POST .../platform-locations`が201で成功することを確認。確認後、複製元・
      複製先とも削除して後始末した）
- [x] 位置未入力のコンコースが警告として出ること（自動テストで確認済みに加え、
      2026-09-14に「位置を入力」導線もブラウザで手動確認。アクセス点のx座標を
      空欄のままコンコースを保存すると「位置未登録の設備・乗換」セクションに
      表示され、「位置を入力」ボタンを押すとホーム中央値（100m）が
      `xPositionMeters`欄に補完されること、保存すると`PUT .../platform-locations/{id}`
      が200で成功し警告セクションから消えることを確認）
- [x] Neon `development` で実際に新規コンコース作成・複製・保存・削除の一連を確認
      （2026-09-14、Chrome DevTools MCP経由でブラウザ手動確認。渋谷
      東京メトロ銀座線1番線で新規作成→複製→削除×2まで一連の操作を行い、
      いずれも該当APIが成功ステータスを返すことを確認。書き込み後は
      コンコース0件の状態に戻し、テストデータを残さなかった）
...
- 新規コンコース・新規停車パターンの作成は同時に1件のみ保持する（複数の
  ...
  次の新規作成に進む）
- [x] ホーム削除ボタンを追加。削除確認モーダルからの手動確認は2026-09-14に実施
      （下記Phase 14参照）
...
- [x] 後続Issueとして起票を検討（起票は開発者判断のため記録に留める）:
      - **[admin] 設備アグリゲートの書き込みを全置換から部分更新へ改める**:
        ...
        **本PRで解消済みのため取り下げた項目**: 「`duplicate` の読みを
        ...
        （PR4で対応済み）
- [x] ブラウザでの手動確認（複製・新規作成・削除の一連、「位置を入力」導線、
      ホーム削除モーダル）を2026-09-14に実施（PR4から持ち越し）。Chrome DevTools MCP
      でNeon `development`（`.env.local`のDATABASE_URLが指す環境。書き込み許可は
      開発者確認済み）に接続し、渋谷 東京メトロ銀座線1番線で以下を確認した:
      - 新規コンコース作成: アクセス点の位置を空欄のまま保存すると
        `POST .../platform-locations`が201で成功し、「位置未登録の設備・乗換」
        セクションに警告として表示される
      - 「位置を入力」導線: ボタン押下でホーム中央値（100m）が補完され、保存で
        `PUT .../platform-locations/{id}`が200、警告セクションから消える
      - 複製 → x をずらす → 保存: 複製するとx座標が+2m（100m→102m）ずれた状態で
        編集フォームに反映され、そのまま保存で`POST`が201
      - 削除: コンコース削除で`DELETE .../platform-locations/{id}`が200
      - ホーム削除モーダル: 一時テスト用ホーム「テスト99番線」を追加して
        「このホームを削除」を押すと確認モーダルが表示され、「削除する」で
        `DELETE .../platforms/{id}`が200
      検証後はテストデータ（コンコース・ホームとも）をすべて削除し、
      駅データを検証前の状態（コンコース0件、1番線/2番線のみ）に戻した
- [x] **2026-09-14訂正**: 上記は誤り。`development`に表参道 東京メトロ半蔵門線3番線
      （反転編成、10両編成）が既に存在することを開発者から指摘を受け確認した
      （詳細はPhase 9末尾の追記を参照）。同駅同ホームでキーボード操作による境界編集を
      ブラウザ手動確認し、隣接カー以外の座標が無変化のまま保たれることを確認した
- [x] **2026-09-14追記**: 反転編成のドラッグ系E2Eを`e2e/station-layout.spec.ts`に追加した
      （詳細はPhase 9末尾の追記を参照）。PR3から持ち越しだった引き継ぎ事項は解消した。
      単体テスト（`editDraft.test.ts`/`DiagramEditLayer.test.tsx`）の反転編成ケースと
      合わせて、ロジック単体・実データ経由のブラウザ操作の両方で反転編成の回帰を検知
      できる状態になった
